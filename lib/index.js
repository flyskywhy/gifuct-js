...
var _imageData = _interopRequireDefault(require("@canvas/image-data"));
...
var generatePatch = function generatePatch(image) {
  ...
  for (var i = 0; i < totalPixels; i++) {
    var pos = i * 4;
    var colorIndex = image.pixels[i]; // let the transparent RGBA just be 0 to be the same behavior of
...
var generateImageData = function generateImageData(parsedGif, resultImages) {
  ...
  for (var i = 0; i < resultImages.length; i++) {
    ...
    if (width * height * 4 === resultImage.patch.length) {
      resultImage.imageData = new _imageData["default"](resultImage.patch, width, height);
    } else if (i > 0) {
      var imageData = new _imageData["default"](new Uint8ClampedArray(resultImages[i - 1].imageData.data), width, height);
      ...
      for (var _y = 0; _y < dimsHeight; _y++) {
        for (var _x = 0; _x < dimsWidth; _x++) {
          ...
          if (patch[index + 3] > 0) {
            var _op = ((_y + top) * width + _x + left) * 4;

            imageData.data[_op++] = patch[index++];
            imageData.data[_op++] = patch[index++];
            imageData.data[_op++] = patch[index++];
            imageData.data[_op] = patch[index];
          }
